...
// record NONCE 5
#define DT5 "2025-01-12T04:00:00.000000"
#define HN5 "somehost5"
#define UN5 "someuser5"
#define S5 "source5"
#define UUID_5 "51234567-89AB-CDEF-0123-456789ABCDEF"
...
extern "C" void setup()
{
	...
	records[4] = jaldb_create_record();
	records[4]->version = EXPECTED_RECORD_VERSION;
	records[4]->type = JALDB_RTYPE_JOURNAL;
	records[4]->timestamp = jal_strdup(DT5);
	records[4]->hostname = jal_strdup(HN5);
	records[4]->source = jal_strdup(S5);
	records[4]->username = jal_strdup(UN5);
	records[4]->payload = jaldb_create_segment();
	assert_equals(0, uuid_parse(UUID_5, records[4]->uuid));
}
...
extern "C" void test_journal_resume()
{
	struct jaldb_record *rec = NULL;
	char *nonce = NULL;
	assert_equals(JALDB_OK, jaldb_insert_record(context, records[4], 1, &nonce, -1));
	assert_equals(JALDB_OK, jaldb_get_record(context, JALDB_RTYPE_JOURNAL, nonce, &rec));

	uint64_t offset = 0;
	char* testPathStr = (char*)"/test";
	char* outPath = NULL;
	char* outNonce = NULL;

	assert_equals(JALDB_E_DB, jaldb_get_journal_resume(context, HN5, &outNonce, &outPath, offset));
	assert_equals(0, offset);
	assert_equals(NULL, outPath);
	assert_equals(NULL, outNonce);

	offset = 15;
	assert_equals(JALDB_OK, jaldb_store_journal_resume(context, HN5, nonce, testPathStr, offset));
	offset = 0;
	assert_equals(JALDB_OK, jaldb_get_journal_resume(context, HN5, &outNonce, &outPath, offset));
	assert_equals(15, offset);
	assert_equals(0, strcmp(testPathStr, outPath));
	assert_equals(0, strcmp(nonce, outNonce));

	assert_equals(JALDB_OK, jaldb_clear_journal_resume(context, HN5));
	offset = 0;
	free(outPath);
	free(outNonce);
	outPath = NULL;
...
enum jaldb_status jaldb_insert_record_force_time(jaldb_context *ctx, struct jaldb_record *rec, int confirmed, char **local_nonce, char* ts)
{
	...
	auto txn = rdbs->primary_db->getRWTransaction();
	try {
		...
		{
			fprintf(stderr, "ERROR: Insert Record Force Time - Failed to insert record: %s\n", err.what());
			txn.abort();
			ret = JALDB_E_DB;
			return ret;
		}
		...
		{
			fprintf(stderr, "ERROR: Insert Record Force Time - Failed to commit transaction: %s\n", err.what());
			txn.abort();
			ret = JALDB_E_DB;
			return ret;
		}
	...
	{
		fprintf(stderr, "ERROR: Insert Record Force Time - exception: %s\n", err.what());
		txn.abort();
		return JALDB_E_DB;
	}
	catch (...)
	{
		fprintf(stderr, "ERROR: Insert Record Force Time - unknown error occurred\n");
		txn.abort();
		return JALDB_E_DB;
	}
...
//TODO lmdb - this is expected to fail until the jald_context method is implemented (indexes)
...
//	records[0]->network_nonce = jal_strdup(NONCE1);
	...
	fprintf(stderr, "nonce: %s, nonce2: %s\n", nonce, nonce2);
	assert_not_equals(0, strcmp(nonce,nonce2));
...
extern "C" void test_db_destroy_sets_ctx_to_null()
{
	...
	enum jaldb_status ret = jaldb_context_init(ctx, OTHER_DB_ROOT);
	assert_equals(JALDB_OK, ret);
	jaldb_context_destroy(&ctx);
...
extern "C" void test_read_only_flag_prevents_writing_to_db()
{
	...
	enum jaldb_status ret;
	int db_err;
	ret = jaldb_context_init(ctx, OTHER_DB_ROOT);
	assert_equals(JALDB_OK, ret);
