		...
		DOMDocument *doc, DOMElement **elem)
{
	...
	ret = jalls_create_base64_element(doc, digest_buf, len, namespace_uri, DIGESTVALUE, &digestvalue_elem);
	if (ret != JAL_OK) {
		goto err_out;
	}
...
XMLCh *jalls_get_xml_x509_serial(ASN1_INTEGER *i)
{
	...
	XMLCh *xml_serial = jalls_BN2decXMLCh(bn_buff);
		...
		DOMElement *parent_element, DOMElement *last_element, const XMLCh *id)
{
	...
	XMLCh *xml_rsa_modulus = jalls_BN2b64(new_rsa->n);
	XMLCh *xml_rsa_exponent = jalls_BN2b64(new_rsa->e);
	...
	if (x509) {
		...
		XMLCh *xml_subject = jalls_get_xml_x509_name(X509_get_subject_name(x509));
		XMLCh *xml_issuer = jalls_get_xml_x509_name(X509_get_issuer_name(x509));
		XMLCh *xml_serial = jalls_get_xml_x509_serial(X509_get_serialNumber(x509));
		XMLCh *xml_cert = jalls_get_xml_x509_cert(x509);
