...
extern "C" int jalls_handle_audit(struct jalls_thread_context *thread_ctx, uint64_t data_len, uint64_t meta_len)
{
	...
	db_err = jaldb_insert_record(thread_ctx->db_ctx, rec, 1, &nonce, thread_ctx->audit_record_size_limit);
	free(nonce);
	nonce = NULL;
	if (JALDB_OK != db_err) {
		...
		switch (db_err) {
			case JALDB_E_REJECT:
				fprintf(stderr, "Audit record rejected due to record being larger than record size limit of %lld\n", thread_ctx->audit_record_size_limit);
				break;
