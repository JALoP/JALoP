...
extern "C" void test_create_base64_element_fails_does_not_overwrite_existing_elm_pointer()
{
	...
	ret = jalls_create_base64_element(doc, (uint8_t *) base64_input_str, strlen(base64_input_str), namespace_uri, tag, &new_elem);
...
extern "C" void test_create_reference_elem_returns_null_with_null_inputs()
{
	DOMElement *new_elem = NULL;
	jal_status ret;
...
extern "C" void test_create_reference_elem_succeeds_with_no_uri()
{
	DOMElement *reference_elem = NULL;
	DOMElement *digest_method_elem = NULL;
	DOMElement *digest_value_elem = NULL;
	...
	ret = jalls_create_reference_elem(NULL, (char *) EXAMPLE_DIGEST_METHOD,
			(uint8_t *)base64_input_str, strlen(base64_input_str), doc, &reference_elem);
	assert_equals(JAL_OK, ret);
	assert_not_equals(NULL, reference_elem);
	assert_tag_equals(REFERENCE, reference_elem);
...
extern "C" void test_create_reference_elem_fails_does_not_overwrite_existing_pointer()
{
	DOMElement *reference_elem = NULL;
	jal_status ret;
...
extern "C" void test_create_reference_elem_fails_bad_url()
{
	DOMElement *reference_elem = NULL;
	jal_status ret;
...
extern "C" void test_digest_xml_data_returns_inval_for_null()
{
	...
	uint8_t *dgst = NULL;
	int dgst_len = 0;
	enum jal_status ret = jalls_digest_xml_data(NULL, NULL, NULL, NULL);

	ret = jalls_digest_xml_data(NULL, doc, &dgst, &dgst_len);
	assert_not_equals(ret, JAL_OK);
	assert_equals(dgst, NULL);
	assert_equals(0, dgst_len);

	ret = jalls_digest_xml_data(dgst_ctx, NULL, &dgst, &dgst_len);
	assert_not_equals(ret, JAL_OK);
	assert_equals(dgst, NULL);
	assert_equals(0, dgst_len);

	ret = jalls_digest_xml_data(dgst_ctx, doc, NULL, &dgst_len);
	assert_not_equals(ret, JAL_OK);
	assert_equals(dgst, NULL);
	assert_equals(0, dgst_len);

	ret = jalls_digest_xml_data(dgst_ctx, doc, &dgst, NULL);
	assert_not_equals(ret, JAL_OK);
	assert_equals(dgst, NULL);
	assert_equals(0, dgst_len);
}

extern "C" void test_digest_xml_data_returns_inval_for_bad_digest_ctx()
{
	generate_doc_for_canon();
	uint8_t *dgst = NULL;
	int dgst_len = 0;
	...
	enum jal_status ret = jalls_digest_xml_data(dgst_ctx, doc, &dgst, &dgst_len);
	assert_not_equals(ret, JAL_OK);
	assert_equals(dgst, NULL);
	assert_equals(0, dgst_len);
	...
	dgst_ctx = jal_sha256_ctx_create();
	dgst_ctx->init = NULL;
	ret = jalls_digest_xml_data(dgst_ctx, doc, &dgst, &dgst_len);
	assert_not_equals(ret, JAL_OK);
	assert_equals(dgst, NULL);
	assert_equals(0, dgst_len);
...
extern "C" void test_digest_xml_data_returns_inval_for_allocated_dgst_buffer()
{
	generate_doc_for_canon();
...
extern "C" void test_digest_xml_data_canonicalizes_and_digests()
{
	generate_doc_for_canon();
	uint8_t *dgst = NULL;
	int dgst_len = 0;
...
extern "C" void test_create_audit_transforms_elem_null_inputs()
{
	DOMElement *elem = NULL;
	enum jal_status ret;
	ret = jalls_create_audit_transforms_elem(NULL, NULL);
	assert_equals(JAL_E_XML_CONVERSION, ret);

	ret = jalls_create_audit_transforms_elem(doc, NULL);
	assert_equals(JAL_E_XML_CONVERSION, ret);

	ret = jalls_create_audit_transforms_elem(NULL, &elem);
	assert_equals(JAL_E_XML_CONVERSION, ret);
}

extern "C" void test_create_audit_transforms_elem_does_not_overwrite_elem()
{
	DOMElement *elem = (DOMElement *)jal_malloc(4);
	DOMElement *temp = elem;
	enum jal_status ret;
	ret = jalls_create_audit_transforms_elem(doc, &elem);
	assert_equals(JAL_E_XML_CONVERSION, ret);
	assert_equals(elem, temp);
	free(elem);
}

extern "C" void test_create_audit_transforms_elem_outputs_correctly()
{
	DOMElement *transforms_elem = NULL;
	DOMElement *temp;
	enum jal_status ret;

	ret = jalls_create_audit_transforms_elem(doc, &transforms_elem);
	assert_equals(JAL_OK, ret);
...
extern "C" void test_add_signature_block()
{
	...
	// sign document
	ret = jalls_add_signature_block(key, cert, doc, doc->getDocumentElement(), NULL, id_val);
	assert_equals(JAL_OK, ret);
...
extern "C" void test_add_signature_block_works_with_prev()
{
	...
	// sign document
	ret = jalls_add_signature_block(key, cert, doc, doc->getDocumentElement(),
...
extern "C" void test_add_signature_fails_with_bad_input()
{
	...
	assert_not_equals((void *) NULL, key);
	ret = jalls_add_signature_block(NULL, NULL, doc, doc->getDocumentElement(), NULL, id_val);
	assert_equals(JAL_E_INVAL, ret);
	ret = jalls_add_signature_block(key, NULL, NULL, doc->getDocumentElement(), NULL, id_val);
	assert_equals(JAL_E_INVAL, ret);
	ret = jalls_add_signature_block(key, NULL, doc, NULL, NULL, id_val);
	assert_equals(JAL_E_INVAL, ret);
	ret = jalls_add_signature_block(key, NULL, doc, doc->getDocumentElement(), NULL, NULL);
	assert_equals(JAL_E_INVAL, ret);
}
extern "C" void test_add_signature_works_without_cert()
{
	...
	// sign document
	ret = jalls_add_signature_block(key, NULL, doc, doc->getDocumentElement(), NULL, id_val);
	assert_equals(JAL_OK, ret);
