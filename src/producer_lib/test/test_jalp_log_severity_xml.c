...
void test_log_severity_to_elem_success()
{
	...
	enum jal_status ret = jalp_log_severity_to_elem(severity, app_meta_elem, &new_elem);
...
void test_log_severity_to_elem_fails_with_bad_input()
{
	...
	ret = jalp_log_severity_to_elem(severity, app_meta_elem, NULL);
	assert_equals(JAL_E_XML_CONVERSION, ret);

	ret = jalp_log_severity_to_elem(NULL, app_meta_elem, &new_elem);
	assert_equals(JAL_E_XML_CONVERSION, ret);
	assert_equals(1, new_elem == NULL);

	ret = jalp_log_severity_to_elem(severity, app_meta_elem, &new_elem);
	assert_equals(JAL_OK, ret);
