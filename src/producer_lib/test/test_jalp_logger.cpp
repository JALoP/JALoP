/**
 * @file test_jalp_logger.cpp This file contains functions to test jalp_log().
 *
 * @section LICENSE
 *
...
{
	ctx_is_null = !jctx;
	message_type_wrong = (message_type != JALP_LOG_MSG);
	data_is_null = !data;
...
extern "C" void test_log_fails_with_bad_input()
{
	enum jal_status ret;
	ret = jalp_log(ctx, NULL, NULL, 0);
	assert_equals(JAL_E_INVAL, ret);

	ret = jalp_log(ctx, app_meta, NULL, 10);
	assert_equals(JAL_E_INVAL, ret);

	ret = jalp_log(ctx, app_meta, (uint8_t *)BUFFER, 0);
	assert_equals(JAL_E_INVAL, ret);

	ret = jalp_log(NULL, app_meta, (uint8_t*)BUFFER, strlen(BUFFER));
	assert_equals(JAL_E_INVAL, ret);
}

extern "C" void test_log_works_with_good_input()
{
	expected_data_len = strlen(BUFFER);
	expected_meta_len = 1;
	enum jal_status ret = jalp_log(ctx, app_meta, (uint8_t *)BUFFER, strlen(BUFFER));
