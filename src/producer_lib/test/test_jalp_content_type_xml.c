...
void test_jalp_content_type_to_elem_fails_with_bad_input()
{
	...
	ret = jalp_content_type_to_elem(ct, app_meta_elem, NULL);
	assert_equals(JAL_E_XML_CONVERSION, ret);
	assert_equals((void*)NULL, new_elem);

	ret = jalp_content_type_to_elem(NULL, app_meta_elem, &new_elem);
	assert_equals(JAL_E_XML_CONVERSION, ret);
	assert_equals((void*)NULL, new_elem);

	free(ct->subtype);
	ct->subtype = NULL;
	ret = jalp_content_type_to_elem(ct, app_meta_elem, &new_elem);
...
void test_jalp_content_type_to_elem_fails_with_bad_media_type()
{
	xmlNodePtr new_elem = NULL;
	ct->media_type = -1;
	enum jal_status ret = jalp_content_type_to_elem(ct, app_meta_elem, &new_elem);
	assert_equals(JAL_E_INVAL_CONTENT_TYPE, ret);
	assert_equals((void*)NULL, new_elem);
