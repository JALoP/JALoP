...
static xmlNodePtr app_meta_elem = NULL;
...
void setup()
{
	...
	// Create an empty top-level node of the ApplicationMetadata type
	// This will be the parent for nodes generated by the tests
	// Gets destroyed when xmlFreeDoc is called
	app_meta_elem = xmlNewDocNode(doc, NULL, (xmlChar*)APPLICATIONMETADATA, NULL);
...
void test_transform_to_elem_returns_null_with_null_inputs()
{
	...
	ret = jalp_transform_to_elem(transform2, app_meta_elem, NULL);
	...
	ret = jalp_transform_to_elem(NULL, app_meta_elem, &new_elem);
	assert_equals(JAL_E_XML_CONVERSION, ret);
	assert_equals((void*)NULL, new_elem);
}
...
void test_transform_to_elem_fails_does_not_overwrite_existing_elm_pointer()
{
	...
	ret = jalp_transform_to_elem(transform2, app_meta_elem, &new_elem);
	assert_equals(JAL_OK, ret);
	assert_not_equals(NULL, new_elem);

	xmlNodePtr orig = new_elem;
	ret = jalp_transform_to_elem(transform1, app_meta_elem, &new_elem);
	assert_equals(JAL_E_XML_CONVERSION, ret);
...
void test_transform_to_elem_with_bad_inputs_does_not_overwrite_existing_elm_pointer()
{
	...
	ret = jalp_transform_to_elem(transform1, app_meta_elem, NULL);
	assert_equals(JAL_E_XML_CONVERSION, ret);
	assert_equals(orig, new_elem);
}
...
void test_transform_to_elem_succeeds_with_custom()
{
	...
	ret = jalp_transform_to_elem(transform_other, app_meta_elem, &new_elem);
	assert_equals(JAL_OK, ret);
...
void test_transform_to_elem_handle_xor_succeeds_with_key()
{
	...
	ret = jalp_transform_to_elem_handle_xor(transform_elm, namespace_uri, enc_info);
	assert_equals(JAL_OK, ret);
...
void test_transform_to_elem_handle_xor_fails_with_incorrect_inputs()
{
	...
	ret = jalp_transform_to_elem_handle_xor(transform_elm, namespace_uri, NULL);
	assert_equals(JAL_E_INVAL_TRANSFORM, ret);
	assert_not_equals(NULL, transform_elm);
}
...
void test_transform_to_elem_succeeds_with_xor()
{
	...
	ret = jalp_transform_to_elem(transform_xor, app_meta_elem, &new_elem);
	assert_equals(JAL_OK, ret);
	assert_attr_equals("Algorithm",
...
void test_transform_to_elem_handle_aes_succeeds_with_no_key_no_iv()
{
	...
	ret = jalp_transform_to_elem_handle_aes(transform_elm, namespace_uri,
...
void test_transform_to_elem_succeeds_with_128_aes()
{
	...
	ret = jalp_transform_to_elem(transform_aes, app_meta_elem, &new_elem);
	assert_equals(JAL_OK, ret);
...
void test_transform_to_elem_succeeds_with_deflate()
{
	...
	ret = jalp_transform_to_elem(transform_deflate, app_meta_elem, &new_elem);
	assert_equals(JAL_OK, ret);
	assert_attr_equals("Algorithm",
