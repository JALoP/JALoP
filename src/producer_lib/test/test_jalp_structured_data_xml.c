...
void setup()
{
	...
	app_meta_elem = xmlNewDocNode(new_doc, NULL, (xmlChar*)APPLICATIONMETADATA, NULL);
...
void test_jalp_structured_data_to_elem()
{
	...
	enum jal_status ret = jalp_structured_data_to_elem(sd, app_meta_elem, &new_elem);
...
void test_jalp_structured_data_to_elem_fails_with_bad_params()
{
	...
	ret = jalp_structured_data_to_elem(sd, app_meta_elem, NULL);
	...
	ret = jalp_structured_data_to_elem(NULL, app_meta_elem, &new_elem);
	assert_equals(JAL_E_XML_CONVERSION, ret);
	assert_equals(1, new_elem == NULL);
	assert_equals(1, new_doc != NULL);
	...
	free(sd->sd_id);
	sd->sd_id = NULL;
	ret = jalp_structured_data_to_elem(sd, app_meta_elem, &new_elem);
...
void test_jalp_structured_data_to_elem_fails_with_no_param_list()
{
	...
	ret = jalp_structured_data_to_elem(bad_sd, app_meta_elem, &new_elem);
	assert_equals(JAL_E_INVAL_STRUCTURED_DATA, ret);
	assert_equals(1, new_doc != NULL);
	assert_equals(1, new_elem == NULL);
