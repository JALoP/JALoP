...
{
	enum jal_status ret;
	ret = jalpx_audit(ctx, NULL, NULL, 0);
	assert_equals(JAL_E_INVAL, ret);

	ret = jalpx_audit(ctx, app_meta, NULL, 0);
	assert_equals(JAL_E_INVAL, ret);

	ret = jalpx_audit(ctx, app_meta, buffer, 0);
	assert_equals(JAL_E_INVAL, ret);

	ret = jalpx_audit(ctx, app_meta, (uint8_t*)BAD_BUFFER, strlen(BAD_BUFFER));
...
void test_audit_works_with_good_input()
{
	...
	expected_meta_len = 1;
	enum jal_status ret = jalpx_audit(ctx, app_meta, buffer, buff_len);
