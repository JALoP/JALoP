...
void test_syslog_metadata_to_elem_returns_error_on_bad_input()
{
	...
	ret = jalp_syslog_metadata_to_elem(NULL, ctx, app_meta_elem, &new_elem);
	assert_not_equals(JAL_OK, ret);
	assert_equals((void*)NULL, new_elem);

	ret = jalp_syslog_metadata_to_elem(syslog, NULL, app_meta_elem, &new_elem);
	assert_not_equals(JAL_OK, ret);
	assert_equals((void*)NULL, new_elem);
	...
	ret = jalp_syslog_metadata_to_elem(syslog, ctx, app_meta_elem, NULL);
	assert_equals((void*)NULL, new_elem);
	assert_not_equals(JAL_OK, ret);

	// errors for invalid elements.
	syslog->facility = -2;
	ret = jalp_syslog_metadata_to_elem(syslog, ctx, app_meta_elem, &new_elem);
	assert_not_equals(JAL_OK, ret);
	assert_equals((void*)NULL, new_elem);
	...
	assert_equals(-1, validate(doc, __FUNCTION__, TEST_XML_APP_META_SCHEMA, 0));
