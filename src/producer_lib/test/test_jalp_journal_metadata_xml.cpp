...
static DOMElement *new_elem = NULL;
...
void test_jalp_journal_metadata_to_elem_fails_when_file_transform_to_elem_fails()
{
	enum jal_status ret;
	jalp_file_info_destroy(&jmeta->file_info);
	ret = jalp_journal_metadata_to_elem(jmeta, doc, &new_elem);
...
void test_jalp_journal_metadata_to_elem_returns_valid_elm_with_valid_input()
{
	...
	// element.
	DOMElement *file_info = dynamic_cast<DOMElement *>(new_elem->getFirstChild());
	...
	DOMElement *transforms = dynamic_cast<DOMElement *>(file_info->getNextSibling());
	assert_not_equals(NULL, transforms);

	DOMElement * xform1 = dynamic_cast<DOMElement *>(transforms->getFirstChild());
	assert_not_equals(NULL, xform1);
	assert_attr_equals(ALGORITHM_ATTR, XFORM_ONE_URI, xform1);

	DOMElement * xform2 = dynamic_cast<DOMElement *>(xform1->getNextSibling());
...
void test_jalp_journal_metadata_to_elem_returns_valid_elm_with_no_transforms()
{
	...
	DOMElement *file_info = dynamic_cast<DOMElement *>(new_elem->getFirstChild());
	assert_not_equals(NULL, file_info);
	DOMElement *should_be_null = dynamic_cast<DOMElement *>(file_info->getNextSibling());
