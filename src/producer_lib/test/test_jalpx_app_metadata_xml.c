...
void test_app_metadata_to_elem_fails_with_invalid_input()
{
	...
	ret = jalpx_app_metadata_to_elem(NULL, ctx, doc, &new_elem);
	assert_equals(JAL_E_XML_CONVERSION, ret);
	assert_equals(NULL, new_elem);

	ret = jalpx_app_metadata_to_elem(app_meta, NULL, doc, &new_elem);
	assert_equals(JAL_E_XML_CONVERSION, ret);
	assert_equals(NULL, new_elem);

	ret = jalpx_app_metadata_to_elem(app_meta, ctx, NULL, &new_elem);
	assert_equals(JAL_E_XML_CONVERSION, ret);
	assert_equals(NULL, new_elem);

	ret = jalpx_app_metadata_to_elem(app_meta, ctx, doc, NULL);
	assert_equals(JAL_E_XML_CONVERSION, ret);
	assert_equals(NULL, new_elem);

	xmlNodePtr bad_elem = (xmlNodePtr) 0xbadf00d;
	ret = jalpx_app_metadata_to_elem(app_meta, ctx, doc, &bad_elem);
	assert_equals(JAL_E_XML_CONVERSION, ret);
	assert_equals(NULL, new_elem);
}
