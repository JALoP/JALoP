...
{
	enum jal_status ret;
	ret = jalpx_journal_fd(ctx, NULL, -1);
	assert_equals(JAL_E_INVAL, ret);

	ret = jalpx_journal_fd(NULL, NULL, -1);
	assert_equals(JAL_E_INVAL, ret);
...
{
	enum jal_status ret;
	ret = jalpx_journal_path(ctx, NULL, NULL);
	assert_equals(JAL_E_INVAL, ret);
}
void test_journal_path_fails_with_bad_context()
{
	enum jal_status ret;
	ret = jalpx_journal_path(NULL, NULL, "./README");
	assert_equals(JAL_E_INVAL, ret);
}
void test_journal_path_fails_with_bad_file()
{
	enum jal_status ret;
	ret = jalpx_journal_path(NULL, NULL, "./this/file/does/not/exist");
	assert_equals(JAL_E_INVAL, ret);
}
...
void test_jalp_journal_fails_with_bad_input()
{
	...
	ret = jalpx_journal(NULL, NULL, NULL, 0);
	assert_equals(ret, JAL_E_INVAL);

	ret = jalpx_journal(NULL, app_metadata, (uint8_t *)BUFFER, strlen(BUFFER));
	assert_equals(ret, JAL_E_INVAL);

	ret = jalpx_journal(ctx, app_metadata, NULL, strlen(BUFFER));
	assert_equals(ret, JAL_E_INVAL);

	ret = jalpx_journal(ctx, app_metadata, (uint8_t *)BUFFER, 0);
	assert_equals(ret, JAL_E_INVAL);
...
void test_journal_works_with_good_input()
{
	...
	expected_meta_len = 1;
	enum jal_status ret = jalpx_journal(ctx, app_meta, (uint8_t *)BUFFER, strlen(BUFFER));
