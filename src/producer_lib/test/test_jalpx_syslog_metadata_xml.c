...
void test_syslog_metadata_to_elem_returns_error_on_bad_input()
{
	...
	ret = jalpx_syslog_metadata_to_elem(NULL, ctx, doc, &new_elem);
	assert_not_equals(JAL_OK, ret);
	assert_equals(NULL, new_elem);

	ret = jalpx_syslog_metadata_to_elem(syslog, NULL, doc, &new_elem);
	assert_not_equals(JAL_OK, ret);
	assert_equals(NULL, new_elem);

	// an empty entry is not an error, so not adding anything here.

	ret = jalpx_syslog_metadata_to_elem(syslog, ctx, NULL, &new_elem);
	assert_not_equals(JAL_OK, ret);
	assert_equals(NULL, new_elem);

	xmlNodePtr bad_elem = (xmlNodePtr) 0xdeadbeef;
	ret = jalpx_syslog_metadata_to_elem(syslog, ctx, NULL, &bad_elem);
	assert_equals(NULL, new_elem);
	assert_not_equals(JAL_OK, ret);

	ret = jalpx_syslog_metadata_to_elem(syslog, ctx, doc, NULL);
	assert_equals(NULL, new_elem);
	assert_not_equals(JAL_OK, ret);
