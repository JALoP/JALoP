		...
		const size_t audit_buffer_size)
{
	...
	xmlSetGenericErrorFunc(NULL, (xmlGenericErrorFunc)xmlErrHandler);
	validated_doc = xmlReadMemory((const char *)audit_buffer, strlen((const char *)audit_buffer), "", NULL, 0);
	bool isAnXmlDoc = validated_doc != NULL;

	uint8_t flags = jalp_context_get_flags(ctx);

	if((flags & JAF_VALIDATE_XML) && isAnXmlDoc)
	{
		if(!ctx->jaf_validCtxt) // if schema context is not cached
		{
...
// XML parser generic error handler
