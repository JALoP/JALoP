		...
		const size_t audit_buffer_size)
{
	...
	if (app_meta) {
		...
		if (ctx->digest_ctx) {
			status = jalpx_digest_audit_record(ctx->digest_ctx, ctx->schema_root,
			...
			status = jalx_create_audit_transforms_elem(doc, &transforms_elem);
			if (status != JAL_OK) {
				goto out;
			}
		...
		status = jalx_xml_output(doc, &buffer);
		if (status != JAL_OK) {
			goto out;
		}
