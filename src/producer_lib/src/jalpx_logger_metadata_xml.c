/**
 * @file jalpx_logger_metadata_xml.c This file defines functions to handle
		...
		xmlNodePtr *new_elem)
{
	...
	if (logmeta->severity) {
		xmlNodePtr tmp = NULL;
		ret = jalpx_log_severity_to_elem(logmeta->severity, doc, &tmp);
		if (ret != JAL_OK) {
			goto cleanup;
		}
	...
	if (logmeta->stack) {
		...
		while (curr) {
			xmlNodePtr tmp = NULL;
			ret = jalpx_stack_frame_to_elem(curr, doc, &tmp);
			if (ret != JAL_OK) {
