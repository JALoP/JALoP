/**
 * @file jalpx_transform_xml.c This file defines functions to deal with
		...
		xmlNodePtr *out)
{
	...
	*out = NULL;
	...
	case JALP_TRANSFORM_OTHER:
		ret = jalpx_transform_to_elem_handle_custom(&transform_elm,
	...
	case JALP_TRANSFORM_XOR:
		ret = jalpx_transform_to_elem_handle_xor(doc,
					&transform_elm,
					namespace_uri,
					transform->enc_info);
		if (ret != JAL_OK) {
	...
	case JALP_TRANSFORM_AES128:
		ret = jalpx_transform_to_elem_handle_aes(doc,
				&transform_elm,
				namespace_uri,
		...
		const struct jalp_transform_encryption_info *enc_info)
{
	...
	ret = jalx_create_base64_element(doc,
			enc_info->key,
	...
	// FOLLOWING WAS REPLACED BY CODE DIRECTLY BELOW IT
	//
	//xor_elm = xmlNewDocNode(doc, NULL, (xmlChar *)JALP_XML_XOR, NULL);
	//xmlNsPtr ns = xmlNewNs(xor_elm, namespace_uri, NULL);
	//xmlSetNs(xor_elm, ns);
	//xmlAddChild(xor_elm, key_elm);
	//xmlAddChild(*transform_elm, xor_elm);
		...
		const struct jalp_transform_encryption_info *enc_info)
{
	...
	aes_elm = xmlNewChild(
			*transform_elm,
			...
			elm_name,
			NULL);
	//aes_elm = xmlNewDocNode(doc, NULL, elm_name, NULL);
	//xmlNsPtr ns = xmlNewNs(aes_elm, namespace_uri, NULL);
	//xmlSetNs(aes_elm, ns);
	...
	//xmlAddChild(*transform_elm, aes_elm);
