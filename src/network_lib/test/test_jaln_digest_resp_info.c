 ...
 * @file
 *
 * @brief This file contains tests for jaln_digest_resp_info.c functions.
 *
 * ### LICENSE
 *
...
void test_digest_resp_info_create_works_with_valid_input_for_confirmed()
{
	...
	struct jaln_digest_resp_info *di = jaln_digest_resp_info_create(nonce, JALN_DIGEST_STATUS_CONFIRMED);
...
void test_digest_resp_info_create_works_with_valid_input_for_invalid()
{
	...
	struct jaln_digest_resp_info *di = jaln_digest_resp_info_create(nonce, JALN_DIGEST_STATUS_INVALID);
	assert_not_equals((void*)NULL, di);
...
{
	struct jaln_digest_resp_info *di;
	di = jaln_digest_resp_info_create(nonce, JALN_DIGEST_STATUS_CONFIRMED);
...
void test_axl_equals_digest_returns_zero_when_equal() {
	struct jaln_digest_resp_info *di_a = jaln_digest_resp_info_create(nonce, JALN_DIGEST_STATUS_CONFIRMED);
	struct jaln_digest_resp_info *di_b = jaln_digest_resp_info_create(nonce, JALN_DIGEST_STATUS_UNKNOWN);
	assert_equals(0, jaln_axl_equals_func_digest_resp_info_nonce(di_a, di_b));
...
void test_axl_equals_digest_returns_non_zero_when_not_equal() {
	struct jaln_digest_resp_info *di_a = jaln_digest_resp_info_create(nonce, JALN_DIGEST_STATUS_CONFIRMED);
	struct jaln_digest_resp_info *di_b = jaln_digest_resp_info_create(nonce_2, JALN_DIGEST_STATUS_CONFIRMED);
	assert_not_equals(0, jaln_axl_equals_func_digest_resp_info_nonce(di_a, di_b));
	jaln_digest_resp_info_destroy(&di_a);
	jaln_digest_resp_info_destroy(&di_b);
}
...
void test_axl_equals_digest_returns_non_zero_with_bad_input() {
	struct jaln_digest_resp_info *di_a = jaln_digest_resp_info_create(nonce, JALN_DIGEST_STATUS_CONFIRMED);
	struct jaln_digest_resp_info *di_b = jaln_digest_resp_info_create(nonce, JALN_DIGEST_STATUS_CONFIRMED);


	assert_not_equals(0, jaln_axl_equals_func_digest_resp_info_nonce(NULL, di_a));
	assert_not_equals(0, jaln_axl_equals_func_digest_resp_info_nonce(di_a, NULL));

	free(di_a->nonce);
	di_a->nonce = NULL;

	assert_not_equals(0, jaln_axl_equals_func_digest_resp_info_nonce(di_a, di_b));
	assert_not_equals(0, jaln_axl_equals_func_digest_resp_info_nonce(di_b, di_a));
