/**
 * @file test_jaln_subscribe_msg_handler.c This file contains tests for jaln_subscribe_msg_handler.c functions.
 *
 * @section LICENSE
 *
...
void test_process_subscribe_works_with_good_input()
{
	assert_equals(JAL_OK, jaln_process_subscribe((VortexFrame*) 0xbadf00d, &sid));
	assert_not_equals((void*) NULL, sid);
	assert_string_equals("the_sid_string", sid);
}

void test_process_subscribe_fails_when_ct_and_xfr_check_fails()
{
	replace_function(jaln_check_content_type_and_txfr_encoding_are_valid, ct_and_enc_always_fail);
	assert_equals(JAL_E_INVAL, jaln_process_subscribe((VortexFrame*) 0xbadf00d, &sid));
}

void test_process_subscribe_fails_with_wrong_message()
{
	replace_function(vortex_frame_get_mime_header, fake_get_mime_header_bad_msg);
	assert_equals(JAL_E_INVAL, jaln_process_subscribe((VortexFrame*) 0xbadf00d, &sid));
}
void test_process_subscribe_fails_with_missing_message()
{
	replace_function(vortex_frame_get_mime_header, fake_get_mime_header_missing_msg);
	assert_equals(JAL_E_INVAL, jaln_process_subscribe((VortexFrame*) 0xbadf00d, &sid));
}

void test_process_subscribe_fails_with_missing_serial_id()
{
	replace_function(vortex_frame_get_mime_header, fake_get_mime_header_missing_sid);
	assert_equals(JAL_E_INVAL, jaln_process_subscribe((VortexFrame*) 0xbadf00d, &sid));
}

void test_process_subscribe_fails_with_bad_inputs()
{
	assert_not_equals(JAL_OK, jaln_process_subscribe(NULL, &sid));
	assert_not_equals(JAL_OK, jaln_process_subscribe((VortexFrame*) 0xbadf00d, NULL));

	sid = (char*)0xbadf00d;
	assert_not_equals(JAL_OK, jaln_process_subscribe((VortexFrame*) 0xbadf00d, &sid));
	sid = NULL;
}
