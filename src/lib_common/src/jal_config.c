 ...
 * @file
 *
 * @brief This file contains utility functions for reading
 * config files.
 *
 * ### LICENSE
 *
...
int jal_config_lookup_int64(const config_setting_t *setting,
	...
	// Allow extracting an int or and int64 type into an int64 container
	if(config_setting_type(member) == CONFIG_TYPE_INT) {
		// Casting up from int to long long should always be safe
		*field = (long long)config_setting_get_int(member);
	} else if(config_setting_type(member) == CONFIG_TYPE_INT64) {
		*field = config_setting_get_int64(member);
	} else {
		CONFIG_ERROR(setting, name, "should be an int or int64");
		return JAL_CFG_FAILURE;
	}
...
int jal_config_get_elem_group(const config_setting_t *setting, int idx, config_setting_t **group, const char *name) {
	...
	*group = config_setting_get_elem(setting, idx);
	if(!group) {
		CONFIG_ERROR(setting, name, " is not a list/array or index: %d is out of range", idx);
		return JAL_CFG_FAILURE;
	}
