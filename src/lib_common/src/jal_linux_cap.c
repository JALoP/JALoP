...
int modify_capability(cap_value_t cap, cap_flag_value_t on)
{
    ...
    cap_ok = CAP_CLEAR;
    int ret = cap_get_flag(current_process_capabilities_context, cap, CAP_PERMITTED, &cap_ok);
    if (ret != 0) {
        printf("Cannot cap_get_flag\n");
        rc = -1;
        goto out;
    }
    if (cap_ok == CAP_CLEAR) {
    ...
    rc = -1;
    rc = cap_set_flag(current_process_capabilities_context, CAP_EFFECTIVE, 1, &cap, on);
    if (rc != 0) {
        printf("Cannot cap_set_flag\n");
        goto out;
    }
