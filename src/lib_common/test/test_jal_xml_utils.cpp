...
extern "C" void test_jal_create_base64_element_returns_null_with_null_inputs()
{
	...
	ret = jal_create_base64_element(NULL, (uint8_t *) string, strlen(string), namespace_uri, tag, &new_elem);
	assert_equals(JAL_E_INVAL, ret);
	assert_equals(NULL, new_elem);

	ret = jal_create_base64_element(doc, NULL, strlen(string), namespace_uri, tag, &new_elem);
	assert_equals(JAL_E_INVAL, ret);
	assert_equals(NULL, new_elem);

	ret = jal_create_base64_element(doc, (uint8_t *) string, 0, namespace_uri, tag, &new_elem);
	assert_equals(JAL_E_INVAL, ret);
	assert_equals(NULL, new_elem);

	ret = jal_create_base64_element(doc, (uint8_t *) string, strlen(string), NULL, tag, &new_elem);
	assert_equals(JAL_E_INVAL, ret);
	assert_equals(NULL, new_elem);

	ret = jal_create_base64_element(doc, (uint8_t *) string, strlen(string), namespace_uri, NULL, &new_elem);
	assert_equals(JAL_E_INVAL, ret);
	assert_equals(NULL, new_elem);

	ret = jal_create_base64_element(doc, (uint8_t *) string, strlen(string), namespace_uri, tag, NULL);
	assert_equals(JAL_E_INVAL, ret);
	assert_equals(NULL, new_elem);
}
...
extern "C" void test_jal_create_base64_element_fails_does_not_overwrite_existing_elm_pointer()
{
	...
	ret = jal_create_base64_element(doc, (uint8_t *) string, strlen(string), namespace_uri, tag, &new_elem);
