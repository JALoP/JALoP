...
impl Context {
    ...
    pub fn get_next_unsynced_record(&self, rec_type: RecordType, db_path: &Path) -> Result<Option<RecordData>, Error> {
        ...
        let res = match unsafe {
            ...
            ffi::jaldb_status_JALDB_OK => record_data::handle_record(rec_ptr, db_path).map(Some),
    ...
    pub fn get_record(&self, rec_type: RecordType, nonce: &str, db_path: &Path) -> Result<RecordData, Error> {
